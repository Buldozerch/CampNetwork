...
class Settings(Singleton, AutoRepr):
    def __init__(self):
        ...
        self.twitter_delay_actions_min = (
            json_data.get("twitter", {}).get("delay_between_actions", {}).get("min", 60)
        )
        self.twitter_delay_actions_max = (
            json_data.get("twitter", {})
            .get("delay_between_actions", {})
            .get("max", 180)
        )

        self.twitter_delay_quests_min = (
            json_data.get("twitter", {}).get("delay_between_quests", {}).get("min", 300)
        )
        self.twitter_delay_quests_max = (
            json_data.get("twitter", {}).get("delay_between_quests", {}).get("max", 600)
        )
        ...
        self.quest_delay_min = (
            json_data.get("quests", {}).get("delay_between_quests", {}).get("min", 20)
        )
        self.quest_delay_max = (
            json_data.get("quests", {}).get("delay_between_quests", {}).get("max", 40)
        )
        ...
        self.referrals_use_random_from_db = json_data.get("referrals", {}).get(
            "use_random_from_db", True
        )
        self.referrals_use_only_file_codes = json_data.get("referrals", {}).get(
            "use_only_file_codes", False
        )
        # Настройки ресурсов
